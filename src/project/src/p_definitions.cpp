...
typedef struct {
	...
	UINT64			countSeen;
	UINT64 			numIns;
	bool 			HasFallThrough; //INS_HasFallThrough() BBL_HasFallThrough()
	UINT64			countFallTrhough;
	UINT64			countJumped;
	ADDRINT 		fallThroughAAdress;
	ADDRINT 		jumpAddress;
	map<ADDRINT, UINT64> PrevBblMap; // counts for each bbl which bbl called it
	map<ADDRINT, UINT64> nextBblMap; // counts for each bbl which bbl it called
	ADDRINT 		rtnAddress;
} bbl_info_t;
