...
// This map is used to store the RTN information
map<ADDRINT, rtn_info_t> routineMap;

// This map is used to store the CALL instruction information
map<ADDRINT, call_info_t> callInstructionMap;

// This map is used to store the BBL information
...
// Function calls we want to inline, <call_address, target_address>
map<ADDRINT, ADDRINT> calls_to_inline_Map;

// Routines we want to commit , <rtn_address, translated_rtn_num>
map<ADDRINT, int> routines_to_commit_Map;

// The reordered bbls for each routine
map<ADDRINT, vector<ADDRINT>> reorder_Map;

// Routines we want to translate (for inline or reorder)
unordered_set<ADDRINT> routines_to_translate_Uset;

// Unconditional jump instructions we want to add , <after_which_address , jump_address>
map<ADDRINT, uncond_jmp_info_t> uncond_jmps_Map;

// Conditional jump instructions we want to revert, <address , jump_address>
